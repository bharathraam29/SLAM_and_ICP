...
def read_canonical_model(model_name, base_path= None):
  '''
  Read canonical model from .mat file
  model_name: str, 'drill' or 'liq_container'
  return: numpy array, (N, 3)
  '''
  model_fname = os.path.join('./data', model_name, 'model.mat') if not base_path else os.path.join(base_path,'data', model_name, 'model.mat')
...
def load_pc(model_name, id, base_path= None):
  '''
  ...
  return: numpy array, (N, 3)
  '''
  pc_fname = os.path.join('./data', model_name, '%d.npy' % id)  if not base_path else os.path.join(base_path, 'data', model_name, '%d.npy' % id)
  pc = np.load(pc_fname)
